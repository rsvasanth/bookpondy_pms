    ...
    <Tooltip delayDuration={0}>
      <TooltipTrigger asChild>
        ...
        >
          ...
          >
            <TrashIcon size={12} className="text-destructive-foreground" />
          </Button>
          ...
          {event.CustomEventComponent ? (
            ...
            >
              <div className={cn("flex w-full overflow-hidden", event?.minmized ? "items-center flex-row gap-1" : "flex-col")}>
                <div className={cn("font-bold truncate", event?.minmized ? "text-[10px] flex-1" : "text-xs mb-0.5")}>
                ...
                {!event?.minmized && event?.description && (
                  <div className="my-1.5 text-xs text-slate-600 line-clamp-2">{event?.description}</div>
                )}

                {!event?.minmized && (
                  <div className="text-[10px] space-y-0.5 mt-1.5 opacity-80">
      ...
      <TooltipContent side="right" className="p-3 w-64 shadow-xl border-slate-200 bg-white" sideOffset={10}>
        <div className="space-y-2">
          <h4 className="font-bold text-sm text-primary">{event?.title || "Untitled"}</h4>
          {event?.description && (
            <p className="text-xs text-muted-foreground line-clamp-4 leading-relaxed">{event?.description}</p>
          )}
